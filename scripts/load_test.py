...
from corpmind.agents.matching import phase_a_node, phase_b_node, prepare_batch_index
...
class GraphAdapters:
    def __init__(self, client):
        ...
        self._llama_8b_lock = asyncio.Semaphore(1)
...
def temp_consistency_fn(item: dict):
    """Temporary stub for consistency merge logic."""
...
async def load_test(feed_path: Path, n_items: int = 50):
    ...
    if flagged_items:
        ...
        source_idx = (
            sample_item.get("source_row_index", "N/A")
            if isinstance(sample_item, dict)
            else getattr(sample_item, "source_row_index", "N/A")
        )
        title_val = (
            sample_item.get("title", "N/A")
            if isinstance(sample_item, dict)
            else getattr(sample_item, "title", "N/A")
        )

        eval_record = (
            sample_item.get("evaluation_record", {})
            if isinstance(sample_item, dict)
            else getattr(sample_item, "evaluation_record", None)
        )
...
if __name__ == "__main__":
    asyncio.run(load_test(FEED_PATH, n_items=5))